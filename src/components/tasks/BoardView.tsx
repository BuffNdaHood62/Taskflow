...
export default function BoardView({ tasks, onToggleComplete, onTaskClick, onMoveTask }: BoardViewProps) {
  ...
  return (
    <div className="flex-1 overflow-x-auto overflow-y-hidden px-6 py-4" style={{ maxHeight: 'calc(100vh - 56px)' }}>
      <div className="flex gap-4 h-full min-w-max">
        {columns.map(column => (
          <div
            key={column.id}
            className={cn(
              'flex flex-col w-[300px] rounded-xl transition-all duration-200',
              dragOverColumn === column.id && 'ring-2 ring-[var(--brand)] ring-opacity-50',
            )}
          ...
          >
            ...
            <div className="flex items-center justify-between px-3 py-2.5 mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-small font-semibold text-[var(--text-primary)]">{column.title}</span>
                <span className="text-caption text-mono px-1.5 py-0.5 rounded-full bg-[var(--bg-hover)] text-[var(--text-muted)]">
                  {column.tasks.length}
                </span>
              </div>
              <button className="p-1 rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] transition-colors">
                <Plus size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 space-y-2 overflow-y-auto px-1 pb-4" style={{ scrollbarWidth: 'none' }}>
              {column.tasks.map(task => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      'p-3 rounded-xl border border-[var(--border-default)] cursor-pointer transition-all duration-150 group',
                      'hover:border-[var(--border-hover)] hover:shadow-md hover:-translate-y-0.5',
                  ...
                  >
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <span className={cn('text-small font-medium text-[var(--text-primary)] line-clamp-2', task.completed && 'line-through opacity-60')}>
                        {task.title}
                      </span>
                    </div>
              ...
              {dragOverColumn === column.id && (
                <div className="h-16 rounded-xl border-2 border-dashed border-[var(--brand)] bg-[var(--brand-light)] transition-all animate-pulse" />
              )}

              {column.tasks.length === 0 && !dragOverColumn && (
                <div className="flex flex-col items-center py-8 text-center">
                  <p className="text-caption text-[var(--text-muted)]">No tasks</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex items-start pt-2">
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-small text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)] transition-colors border border-dashed border-[var(--border-default)]">
            <Plus size={16} />
            Add column
          </button>
        </div>
