  ...
  return (
    <div
      className={cn(
        ...
        isHovered ? 'bg-(--bg-hover)' : 'bg-transparent',
        priorityCfg.border,
      )}
    ...
    >
      {/* Drag handle */}
      <div className={cn('shrink-0 mt-0.5 text-(--text-muted) opacity-0 group-hover:opacity-100 transition-opacity cursor-grab')}>
      ...
      <button
        ...
        className={cn(
          'shrink-0 w-4.5 h-4.5 rounded-full border-2 flex items-center justify-center mt-0.5 transition-all duration-200',
          task.completed
            ? 'bg-(--brand) border-(--brand)'
            : 'border-(--border-hover) hover:border-(--brand) hover:bg-(--brand-light)',
        )}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        {task.description && (
          <p className="text-small text-(--text-muted) truncate mt-0.5">{task.description}</p>
        )}
          ...
          {totalSubtasks > 0 && (
            <div className="flex items-center gap-1.5">
              <div className="w-16 h-1.5 rounded-full bg-(--bg-hover) overflow-hidden">
                <div className="h-full rounded-full bg-(--brand) transition-all duration-300" style={{ width: `${(completedSubtasks / totalSubtasks) * 100}%` }} />
              </div>
              <span className="text-caption text-mono text-(--text-muted)">{completedSubtasks}/{totalSubtasks}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <button
        ...
        className={cn('shrink-0 p-1 rounded-md text-(--text-muted) hover:bg-(--bg-hover) transition-all opacity-0 group-hover:opacity-100')}
      >
